...
% FIXME - not sure about this last sentence and the two following points. Perhaps just take the second point, but put it elsewhere
...
% FIXME - in 6, make sure you cover samer's KM - cite our AES paper, for example
...
A particular application of DSP is the study of one-dimensional, time domain signals.  %that in real world often exist as audio signals. 
...
%We can classify the different digital signal processing paradigm in these three main tasks:
...
In practice, we characterize this group of tasks by its input/output relationships. The operation outputs a signal (which may be time, frequency or multidimensional domain) when it is fed with an input signal. % (accepting several domains as well). 
...
% FIXME - I am really not sure about this bit. some sentences are highly unclear, and i think that might really confuse the reader
% If you're going to speak about functional programming, introduce it nicely. I am not sure "semantically oriented view of computations instead of a machine-oriented one" is really a good description of Haskell?
...
% FIXME - refer to our AES paper here - and rely on it for the description of function tabling
...
% FIXME - highly unclear, especially as you introduce CTR afterwards. I think you should rewrite that.
% Also, the export/import of data is really too quickly introduced, and probably not in the right place?
...
% FIXME - perhaps just "transaction logic" would do here - be gentle for the reader:-)
