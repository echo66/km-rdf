/*  $Id: sparql.pl,v 1.1 2007/05/11 13:30:32 yves Exp $
...
sparql_describe(IRI, Module, rdf(IRI, P, O)) :-
	Module:rdf(IRI, P, O).
sparql_describe(IRI,Module,rdf(S,P,IRI)) :-
	Module:rdf(S,P,IRI).
%I should recursively describe bnodes I get, but it will be for later...
%sparql_describe(IRI,Module,rdf(S,P,O)) :-
%	Module:rdf(IRI,_,BNode),
%	rdf_db:rdf_is_bnode(BNode),
%	sparql_describe(BNode,Module,rdf(S,P,O)).
